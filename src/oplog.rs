...
impl OpLog {
    ...
    // Log message
    pub fn log_message(&self, message: &message::ProtocolMessage) {
        // Example: Serialize and log the message to the file
        let serialized_message = serde_json::to_string(message).unwrap();
        let mut log_file = self.lf.try_clone().unwrap();
        writeln!(log_file, "{}", serialized_message).unwrap();
    }
